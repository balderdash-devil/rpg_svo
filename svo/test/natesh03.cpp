// modification of natesh01.cpp.
// saves depth maps as a png file
...
class DepthFilterTest {
 public:
  ...
  void writeDepthImage(const std::string experiment_name);
  void denoiseDepth(const std::string experiment_name);
...
//      vk::blender_utils::loadBlenderDepthmap(dataset_dir+"/depth/" + (*it).image_name_ + "_0.depth", *cam_, depth_ref_);
  ...
  writeDepthImage(experiment_name);
}

void DepthFilterTest::writeDepthImage(const std::string experiment_name)
{
    std::string trace_name;
    trace_name = result_dir_ + "/depth_filter_" + experiment_name + ".png";

    int width = frame_ref_->img_pyr_[0].cols;
    int height = frame_ref_->img_pyr_[0].rows;

    cv::Mat depth = cv::Mat::zeros(height,width,CV_32FC1);
    cv::Mat depthImage(height,width,CV_8UC1);
    for(std::list<ConvergedSeed>::iterator i=results_.begin(); i!=results_.end(); ++i)
    {
      depth.at<float>(i->y_,i->x_)  =static_cast<float>((cam_->cam2world(i->x_, i->y_)*i->depth_)[2]);
    }
    cv::normalize(depth, depthImage, 0, 255, CV_MINMAX, CV_8U);
    cv::imwrite(trace_name.c_str(),depthImage);
}
}  // namespace
